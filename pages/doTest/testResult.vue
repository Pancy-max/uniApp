	...
	<view class="test-wrapper">
		<scroll-view scroll-y="true">
			<view class="test-content">
				...
				<view class="content_analyze block-content" v-for="(item, index) in dimData"
					:key="index">
					<view class="title">
					...
					<view class="dim_score">
						...
						<!-- 二级 开始-->
						<block v-if="item.MiniEvaUserDimension && item.MiniEvaUserDimension.length > 0">
						{{item.chartData.categories}}
							<view v-if="recInfo.reportPicType === 'column'" class="">
								<qiun-data-charts type="column" :chartData="item.chartData" :inScrollView="true" />
							</view>
							<view v-if="recInfo.reportPicType === 'radar'" class="">
								<qiun-data-charts type="radar" :chartData="item.chartData" :inScrollView="true" />
							</view>
							
							<view class="content_analyze block-content" v-for="(ele, i) in item.MiniEvaUserDimension"
								:key="i">
								...
								<view class="dim_score">
									<view class="dim_title">
										<uni-rate :readonly="true" :value="ele.score / 20" class="rate-score" />
			...
			getUserEvaInfo() {
				this.request({
					...
					for (let item of dimData) {
						if (item.MiniEvaUserDimension && item.MiniEvaUserDimension.length > 0) {
							const subDimData = item.MiniEvaUserDimension
							item.chartData = {
								categories: subDimData.map(ele => {
									return ele.subDim
								}),
								series: [
									{
										name: '分数',
										data: subDimData.map(ele => ele.score)
									}
								]
							}
					...
					this.chartData.series[0].data = dimData.map(item => item.score)
				})
